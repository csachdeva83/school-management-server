...
export default class MarksController extends BaseApi {
    ...
    public register(express: Application): void {
        ...
        this.router.get('/performance-percentage', authenticateToken, this.getPerformancePercentage);
    ...
    public getPerformancePercentage = async (req: Request, res: Response<ApiResponse<MarksPerformacePercentage[]> | ApiError>) => {
        try {

            logger.info('Get performance percentage');

            const marks = await this.marksService.getPerformancePercentage(String(req?.query?.studentId),res?.locals?.schoolId);
    
            res.status(StatusCodes.OK)
                .send(new ApiResponse(StatusCodes.OK, 'SUCCESS', marks, 'Student performance percentage for all exams'));
